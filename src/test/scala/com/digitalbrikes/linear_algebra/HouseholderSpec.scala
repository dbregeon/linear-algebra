...
class HouseholderSpec extends Specification {
	"householder " should {
		...
		"compute the householder decomposition for the vector (12.76204, -0.10021, 1.97157) for index 1" in {
			val v = Vector(Nat(3), _Vector(12.76204, -0.10021, 1.97157))
			val expectedStd = Matrix(Nat(3), Nat(3), _Vector(1, 0, 0, 0, BigDecimal("-0.050761985220916399430455403892579"), BigDecimal("0.9987107793833164916186304825116618"), 0, BigDecimal("0.9987107793833164916186304825116618"), BigDecimal("0.0507619852209163994304554038925830")))
			
			householder(v, 1) must be_==(expectedStd)
		}
		
		"compute the householder decomposition for the vector (-8.8818e-16, -4.8303e-01, 4.8572e-16) for index 2" in {
			val v = Vector(Nat(3), _Vector(-8.8818e-16, -4.8303e-01, 4.8572e-16))
			val expectedStd = Matrix(Nat(3), Nat(3), _Vector(1, 0, 0, 0, 1, 0, 0, 0, -1))
			
			householder(v, 2) must be_==(expectedStd)
		}
		
		"compute the householder decomposition for the vector (0, 0, 0) for index 0" in {
			val v = Vector(Nat(3), _Vector(0, 0, 0))
			val expectedStd = Matrix(Nat(3), Nat(3), _Vector(1, 0, 0, 0, 1, 0, 0, 0, 1))
		...
		"compute the householder decomposition for the vector (4.518227016649075E+307, 1.0, -1.0, 1.0) for index 0" in {
			val v = Vector(Nat(3), _Vector(BigDecimal("4.518227016649075E+307"), 1.0, -1.0, 1.0))
			val expectedStd = Matrix(Nat(3), Nat(3), _Vector(1, 0, 0, 0, 1, 0, 0, 0, 1))
			val I = identity(v.rows)
			val H = householder(v, 0)
